...
class Order extends Admin
{
	...
	public function _initialize(){
		...
		$this->list_tab =  [
        'base' => ['title' => '返现订单', 'url' => url('index', ['group' => 'base'])],
        'nopay' => ['title' => '未返现订单', 'url' => url('index', ['group' => 'nopay'])],
		'all' => ['title' => '所有订单', 'url' => url('index', ['group' => 'all'])],
		];
	}
    ...
    {	
		 ...
		 // 查询
			...
			$this->group = $group;
			if($group == "nopay"){
				$data_list = $order->where($map)->where("opuid = 0")->order('update_time desc')->paginate();
			}else if($group == "all"){
				$data_list = $order->where($map)->order('update_time desc')->paginate();				
			}else{
				//$data_list = $order->where($map)->where("mobile <> ''")->order('update_time desc')->paginate();
				$data_list = $order->where($map)->where("opuid > 0")->order('update_time desc')->paginate();
				$this->group = "base";
			}
			...
			return $this->baseTable($data_list);
	}
	
	protected function baseTable($data_list = []){

			
	        return ZBuilder::make('table')
			->setTabNav($this->list_tab,  $this->group)
